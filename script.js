...
function clic1() { // Au clic du bouton 1
  ...
  if ( nbr == 0) { // si ce nombre est égal à 0
    ...
    document.getElementById('s1').innerHTML = msg; // ecraser le contenu de la section existante avec les variables msg et form, en utilisant la méthode .html()
...
function clic2(){ // au clic du bouton 2
  ...
  if(mdp == pswd) { // si le code crée et celui entré par l'utilisateur correspondent
    document.querySelector('s1').innerHTML = '<p>Servez-vous en '+ carbchoisi + '</p>'; // alors écraser une fois de plus le contenu de la section
